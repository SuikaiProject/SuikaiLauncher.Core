import json
import aiohttp
from suikailauncher.core.base import track,version
import time as t
import asyncio
...
class HttpResponse:
    ...
    def is_error(self) -> bool:
        return self.status in http_err
    def json(self) -> dict:
        return json.loads(self.decode("utf-8"))
    def decode(self,encoding:str = "utf-8") ->str:
        return self.data.decode(encoding)
# 万能的网络请求方法
async def network_request(url:str,method:str="GET",headers:dict|None = None,data:str|bytes|None = None,verify_ssl:bool = True,retry:int = 5,use_browser_ua:bool = False,max_redirect:int = 20,use_stream:bool = True):
    global session
    if not session:
    ...
    try:
        ...
        while redirect <= max_redirect:
            ...
            async with session.request(method.upper(),request_url,headers=request_headers,data=data,verify_ssl=verify_ssl,allow_redirects=False) as resp:                
                ...
                # 反正没法处理（description 也不太想写 match case），先摆再说
                except Exception as e:
                    if retried > retry:
                        break
                    logger.error(track.get_ex_summary(e))
                    retried += 1
                    continue
        # 古希腊掌管重定向的神（
